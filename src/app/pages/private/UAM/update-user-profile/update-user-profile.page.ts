...
export class UpdateUserProfilePage implements AfterViewInit {
  ...
  public cityValidationMessage: string = '';
  ...
  UpdateButtonClicked() {
    if (this.userService.Validate(this.user)) {
      this.checkUserNameExistance(
        () => {
          this.presentAlert(
            this.languageService.translate('SIGN_UP.ALREADY_REGISTERED'),
            this.languageService.translate(
              'SIGN_UP.USER_WITH_USERNAME_ALREADY_EXISTS'
            )
          );
        },
        () => {
