...
export class SettingsPage implements OnInit, AfterViewInit {
  ...
  serveBasedOnUserRole() {
    //Role wise changes
    this.loggedInUser = this.localStorageService.StoredPreference.LoggedInUser??new User({});
  }
  loadLocalizationDetails()
  {
    this.languageSelected = this.localStorageService.StoredPreference.SelectedLanguage??"en";
  ...
  updateSelectedLanguageLabel()
  {
    ...
    //Loaders
    async initializeLoader(message:string) {
