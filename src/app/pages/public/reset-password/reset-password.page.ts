...
export class ResetPasswordPage implements AfterViewInit {
  ...
  newPasswordTextChanged() {
    ...
    this.passwordStrength = HelperService.getPasswordStrength(this.newPassword?this.newPassword:"");
  ...
  async resetButtonClicked() {
    ...
    if (this.validate()) {
      ...
      this.userService.checkUser(this.email?this.email:"", this.oldPassword?this.oldPassword:"", (response:any) => {

        this.userService.ResetPassword(response.id, this.newPassword?this.newPassword:"", () => {
