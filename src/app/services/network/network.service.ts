...
export class NetworkService {
  ...
  get(url: string, success: any, failure: any) {
    ...
    if (url.indexOf('sheets/Users') > -1) {
      headers = headers.append(
        KeywordConstants.SPREADSHEET_ID_KEY,
        environment.SpreadSheetIdUser
      );
    } else {
      headers = headers.append(
        KeywordConstants.SPREADSHEET_ID_KEY,
    ...
    headers = headers.append(
      KeywordConstants.AUTH_TOKEN_KEY,
      'Bearer ' + environment.READ_ONLY_API_KEY
  ...
  patch(url: string, data: any, success: any, failure: any) {
    ...
    let headers = new HttpHeaders();
    headers = headers.append('Content-Type', 'application/json;');
    this.httpClient.patch(url, data, { headers }).subscribe(
      (response) => {
        success(response);
      },
      () => {}
  ...
  put(url: string, data: any, success: any, failure: any) {
    ...
    headers = headers.append('Content-Type', 'application/json');
    this.httpClient.put(url, data, { headers }).subscribe(
      (response) => {
        success(response);
      },
      () => {}
  ...
  postFormData(url: string, data: any, success: any, failure: any) {
    ...
    this.httpClient.post(url, data, { headers: headers }).subscribe(
      (response) => {
        success(response);
      },
      () => {
        failure('Log the error here');
      }
    );
  }
}
